...
def GET_GRN(counts, velocity=None, model_to_test = None):
    if velocity is None:
        VIM = GENIE3(counts, output_data=velocity, model_to_test = model_to_test)
    else:
        VIM = GENIE3(counts, model_to_test = model_to_test)
    return VIM
...
def GENIE3(expr_data,output_data=None,gene_names=None,regulators='all',tree_method='RF',
            K='sqrt',ntrees=500,nthreads=1, model_to_test = None):
    ...
    if model_to_test is None: 
        print("No model provided, will estimate one now...")
    else:
        print("getting GRN for provided model")
    ...
    if nthreads > 1:
        ...
        for i in range(ngenes):
            input_data.append( [expr_data,i,input_idx,tree_method,K,ntrees,output_data, model_to_test] )
        ...
        for i in range(ngenes):
            ...
            vi = GENIE3_single(expr_data,i,input_idx,tree_method,K,ntrees,output_data=output_data, model_to_test = model_to_test)
            VIM[i,:] = vi
...
def wr_GENIE3_single(args):
    return([args[1], GENIE3_single(args[0], args[1], args[2], args[3], args[4], args[5], output_data=args[6], model_to_test = args[7])])
    


def GENIE3_single(expr_data,output_idx,input_idx,tree_method,K,ntrees,output_data=None, model_to_test = None):
